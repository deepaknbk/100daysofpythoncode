...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    stateabbrev=us_state_abbrev.get(state_name,NOT_FOUND)
    #print(stateabbrev)
    return stateabbrev
...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
    long_state=max(data, key=len)
    # print(max(data, key=len))
    # for state in data:
    #     if len(state)==len(max(data, key=len)):
    #         long_state.append(state)
    #print(long_state)
    return long_state
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    for abbrev in us_state_abbrev.values():
        comb_list.append(abbrev)
    # sorted_comb_list=sorted(comb_list)
    # sorted_comb_list=sorted_comb_list[0:9]
    #print(sorted_comb_list)
    #print(len(states)-10,len(states))
    #print(sorted(states)[-10:-1])
    final_sorted_list=sorted(comb_list)[0:10]+sorted(states)[-10:]
    #comb_list.append(states[len(states)-10:])
    #print(final_sorted_list)
    return final_sorted_list

print(get_every_nth_state())
print(get_state_abbrev('Iowa'))
